...

{
    ...
    public function obtenerVentasProductos($idempleado, $idrol)
    {
        if ($idrol == 1) {
            ...
            venta_producto.cantidad AS total_cantidad,
            empleado.nombre AS nombre_empleado,
            venta_producto.fecha,
            (producto.precio * venta_producto.cantidad) * (1 - (venta_producto.descuento / 100)) AS total_subtotal
            FROM venta_producto
    ...
    {
        $sql = "SELECT venta_producto.id, venta_producto.p_s, producto.pro_serv, producto.unidad, producto.precio, venta_producto.descuento, venta_producto.cantidad, empleado.nombre AS nombre_empleado, venta_producto.fecha,  
    ...
    public function obtenerVentasServicios()
    {
        ...
        empleado.nombre AS nombre_empleado, atiende, producto.precio, venta_servicio.descuento, venta_servicio.cantidad, (producto.precio * venta_servicio.cantidad)  * (1 - (venta_servicio.descuento / 100)) AS subtotal FROM venta_servicio 
    ...
    {
        $fechaHoy = date("Y-m-d");
        $sql = "SELECT SUM(producto.precio * venta_servicio.cantidad) AS total
        FROM venta_servicio
        INNER JOIN producto ON venta_servicio.p_s = producto.id
    ...
    {
        $fechaHoy = date("Y-m-d");
        $sql = "SELECT  SUM( producto.precio * venta_producto.cantidad) AS total
        FROM venta_producto
        INNER JOIN producto ON venta_producto.p_s = producto.id
    ...
    public function resumenVentasXdia()
    {
       ...
       (SUM(producto.precio * vp.cantidad) * (1 - (vp.descuento / 100))) - (SUM(producto.compra * vp.cantidad)) AS ganancia,
        producto.cantidad, 
        vp.fecha 
        FROM ( 
        SELECT p_s, descuento, cantidad, fecha FROM venta_producto 
        UNION ALL SELECT p_s, descuento, cantidad, fecha FROM venta_servicio ) AS vp 
        INNER JOIN producto ON vp.p_s = producto.id 
        GROUP BY vp.p_s, producto.precio, vp.descuento, vp.cantidad, DATE(vp.fecha)";
        $stmt = $this->conection->prepare($sql);
        $stmt->execute();
        $results = $stmt->get_result();
        return $results->fetch_all(MYSQLI_ASSOC);
